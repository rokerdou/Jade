    ...
    const uint32_t lock_time, const uint32_t sequence, char* const output, const size_t output_len)
{
    ...
    if (pos == 0 && sequence != UINT32_MAX) {
        return bitcoin_append_str(output, output_len, &pos, "seq ") && bitcoin_append_hex_u32(output, output_len, &pos, sequence);
    }
    return true;
}
...
bool bitcoin_confirm_summary_from_request(const bitcoin_confirm_request_t* const request, chain_confirm_summary_t* const summary)
{
    ...
    if (!bitcoin_format_sats(request->amount, amount, sizeof(amount))
        || !bitcoin_format_sats(request->self, self, sizeof(self))
